...
describe('A tile', function() {
	...
	beforeAll(function() {
		...
		this.tile = utils.renderIntoDocument(<Tile key={0}
		                                           value={this.val.elements.get('tiles').get(JSON.stringify(key))}/>);
	});
	...
	describe('Desert tile', function() {
		beforeEach(function() {
			...
			this.desertTile = utils.renderIntoDocument(<Tile key={0}
			                                                 value={this.val.elements.get('tiles').get(JSON.stringify(key))}/>);
		});
