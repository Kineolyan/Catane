...
import { MockSocket } from 'server/core/com/mocks.js';

function createPlayer(...args) {
	return new CatanePlayer(new BasePlayer(...args));
}
...
describe('Tile', function() {
	...
	describe('#distributeResources', function() {
		beforeEach(function() {
			...
			this.colony.owner = createPlayer(new MockSocket().toSocket(), 1);
			this.tile.addCity(this.colony);

			this.city = new City(-1, 1);
			this.city.owner = createPlayer(new MockSocket().toSocket(), 2);
			this.city.evolve();
			this.tile.addCity(this.city);
		});
